# -*- coding: utf-8 -*-
"""
Created on Fri Jul 19 14:02:03 2019
...
def reconstruct(data_loader, data_set, mask, models, output_folder):
    """
    Parameters
    ----------
    ...
    with progressbar.ProgressBar(max_value=len(data_loader)) as recon_bar:
        for i, data in enumerate(data_loader, 0):
            ...
            outputs = out.reshape([n_t, n_ch, n_lat*n_lon])
            #Applying mask
            outputs[:, :, ~mask] = np.nan
            #Reshaping Back
            ...
            #Saving Data
            np.save('{}/Chl_{}{:02d}.dat'.format(output_folder, year, mon),
                   outputs)
            recon_bar.update(i)
...
def get_args():
    ...
    parser.add_argument('--models-folder', '-m', required=True, type=str,
...
def main():
    ...
    #Reconstruction
    reconstruct(data_loader, data_set, mask, models, args.output_folder)
